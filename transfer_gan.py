...
def give_me_embeddings(n):
    embd_dict = load_embeddings()
    img_names = embd_dict.keys()
    img_names = [img_names[i] for i in random.sample(range(0, len(img_names)), n)]
    embeddings_selected = [embd_dict[img_name] for img_name in img_names]
    embeddings_selected = np.array(embeddings_selected).astype('float32')
    return embeddings_selected, img_names
...
def GoodGenerator(n_samples, noise=None, dim=DIM, nonlinearity=tf.nn.relu, bn=BN_G, labels=None):
    ...
    fact = N_PIXELS // 16
    output = lib.ops.linear.Linear('Generator.Input', 128 + embedding_size, fact * fact * 8 * dim, noise)
...
def GoodDiscriminator(inputs, dim=DIM, bn=BN_D):
    ...
    # output_acgan = tf.contrib.layers.fully_connected(output, num_outputs=128)
    ...
    if ACGAN:
        output_acgan = lib.ops.linear.Linear('Discriminator.ACGANOutput', fact * fact * 8 * dim, 128, output)
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    fake_labels_global = tf.placeholder('float32', shape=(BATCH_SIZE // len(DEVICES), embedding_size),
                                        name='fake_labels_global')
    for device_index, (device, real_data_conv, real_labels) in enumerate(
            zip(DEVICES, split_real_data_conv, labels_splits)):
    ...
    def generate_image(frame):
        ...
        lib.save_images.save_images(samples.reshape((100, 3, N_PIXELS, N_PIXELS)),
